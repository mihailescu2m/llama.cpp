...
void llama_moe_stream::read_ple_rows(const std::vector<int32_t> & rows, ggml_tensor * dst) {
...

    {
        ...
        for (const auto & [row, i] : reads) {
            q_ple.push_back({ row, ple_buf.data() + i*ple.row_size });
        }
        cv_work.notify_all();

        cv_done.wait(lk, [&] { return load_failed || ple_pending == 0; });
        if (load_failed) {
    ...
    for (size_t i = 0; i < duplicate_of.size(); ++i) {
        if (duplicate_of[i] != SIZE_MAX) {
            memcpy(ple_buf.data() + i*ple.row_size,
                   ple_buf.data() + duplicate_of[i]*ple.row_size,
                   ple.row_size);
        }
    }

    ggml_backend_tensor_set(dst, ple_buf.data(), 0, nbytes);
}
...
void llama_moe_stream_layer::service_requests(llama_moe_stream & mgr) {
    ...
    if (tail[MOE_SLOT_TAIL_NBAD] != 0) {
        // the kernel ran out of evictable slots, or saw an out-of-range expert id
        mgr.load_failed = true;
...
void llama_moe_stream_service_gpu(void * user_data, void * state_host, int32_t layer) {
    GGML_UNUSED(state_host);

    auto * mgr = (llama_moe_stream *) user_data;

    if (llama_moe_stream_layer * sl = mgr->layer(layer)) {
