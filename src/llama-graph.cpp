...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (selected_experts == nullptr) {
        // ggml_argsort_top_k fully sorts all n_expert scores and views the first k. GGML_OP_TOP_K
        // has identical semantics (descending, largest-first) and a dedicated kernel.
    ...
    if (msl && n_stream_waves == 1) {
        // GGML_OP_TOP_K already writes a contiguous tensor; only a view needs the copy
